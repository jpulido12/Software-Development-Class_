...
class mat {
    ...
    void read() // read mat
    {
        for (int i = 0; i < 3; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                cout << "enter value for row " << i << " column "<< j << ": "; // read in values
                cin >> vals[i][j];
            }
        }
    }
    void print() // print mat in form requested
    {
        for (int i = 0; i < 3; i++)
        {
            ...
            {
                cout << vals
                [i][j] << " "; // print out values in the array
            }
            cout << endl;
        }
    }
    ...
    {
        mat m; // create a new mat
        for (int i = 0; i < 3; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                m.set_val(i, j, vals[i][j] * num); // set the values of the new mat to the values of the original multiplied by num
            }
        }
        return m; // return the new mat
    }
    mat operator+(mat m) // operator overloading for addition
    {
        mat m4; //create a new mat
        for (int i = 0; i < 3; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                m4.set_val(i, j, vals[i][j] + m.get_val(i, j)); // set the values of the new mat to the values of the original added to the values of the other mat
            }
        }
        return m4; // return the new mat
    }
};
